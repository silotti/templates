# CHATGPT_RULES
template Concept
VERSAO=1.2

## OBJETIVO

Padronizar a geração de conteúdo utilizando os templates deste repositório.


##teste

---

# ATIVAÇÃO

Estas regras devem ser aplicadas quando o usuário utilizar uma das expressões:

* Template Concept
* Relatório Concept
* Use CHATGPT_RULES

---

# ARQUIVOS DE REFERÊNCIA

## Relatórios

configuracoes/saida_relatorio.md

## Currículos

configuracoes/saida_curriculo.md

## Regras Gerais

configuracoes/concept_rules.md

---

# REGRAS GERAIS

## Idioma

* Utilizar Português do Brasil.
* Utilizar linguagem clara, objetiva e profissional.
* Evitar regionalismos.

## Precisão

* Não inventar informações.
* Quando faltar informação, indicar explicitamente o campo ausente.
* Solicitar dados adicionais quando necessário.

## Formatação

* Utilizar Markdown.
* Utilizar títulos hierárquicos.
* Utilizar listas quando apropriado.
* Utilizar tabelas para informações estruturadas.
* Não utilizar emojis em documentos técnicos.

## Estrutura

* Respeitar rigorosamente os templates fornecidos.
* Alterar apenas os dados variáveis.
* Manter a estrutura fixa dos modelos.

---

# RELATÓRIOS

## Processo

1. Consultar concept_rules.md.
2. Consultar saida_relatorio.md.
3. Aplicar os dados fornecidos.
4. Gerar a saída final.

---

# CURRÍCULOS

## Processo

1. Consultar concept_rules.md.
2. Consultar saida_curriculo.md.
3. Aplicar os dados fornecidos.
4. Gerar a saída final.

---

# PROGRAMAÇÃO PYTHON

## Convenções

Funções:
f_nome_funcao

Parâmetros:
p_nome_parametro

Variáveis locais:
v_nome_variavel

Variáveis globais:
g_nome_variavel

Constantes:
c_nome_constante

## Strings

Utilizar aspas duplas.

Exemplo:

v_nome = "Leonardo"

---

# PRIORIDADE DAS REGRAS

1. Solicitação atual do usuário.
2. Template informado na conversa.
3. Arquivos de saída (saida_relatorio.md, saida_curriculo.md).
4. CHATGPT_RULES.
5. Padrões do modelo.

---

# CONTROLE DE VERSÃO

Quando solicitado pelo usuário, informar:

* Template utilizado.
* Versão utilizada.
* Arquivos consultados.
